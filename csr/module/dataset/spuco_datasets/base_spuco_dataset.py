...
from . import BaseSpuCoCompatibleDataset
...
class BaseSpuCoDataset(BaseSpuCoCompatibleDataset, ABC):
    def __init__(
        ...
        assert (
            split == TRAIN_SPLIT or split == VAL_SPLIT or split == TEST_SPLIT
        ), f"split must be one of {TRAIN_SPLIT}, {VAL_SPLIT}, {TEST_SPLIT}"
        self.split = split
        ...
        if not self.skip_group_validation:
            for class_label in classes:
                for spurious_label in spurious_classes:
                    ...
                    assert (
                        group_label in self._group_partition and len(self._group_partition[group_label]) > 0
                    ), f"No examples in {group_label}, considering reducing spurious correlation strength"
