...
class SpuCoBirds(BaseSpuCoDataset):
    ...
    def load_data(self) -> SourceData:
        ...
        try:
            ...
            assert (
                len(landbirds_land) == MAJORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files [landbirds_land]. Expected {MAJORITY_SIZE[self.split]} files got {len(landbirds_land)}"
            ...
            assert (
                len(landbirds_water) == MINORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files [landbirds_water]. Expected {MINORITY_SIZE[self.split]} files got {len(landbirds_water)}"
            ...
            assert (
                len(waterbirds_land) == MINORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files [waterbirds_land]. Expected {MINORITY_SIZE[self.split]} files got {len(waterbirds_land)}"

            # Waterbirds Water
            waterbirds_water = os.listdir(os.path.join(self.dset_dir, f"{WATERBIRDS}/{WATER}"))
            self.data.X.extend(
                [str(os.path.join(self.dset_dir, f"{WATERBIRDS}/{WATER}", x)) for x in waterbirds_water]
            )
            self.data.labels.extend([1] * len(waterbirds_water))
            self.data.spurious.extend([1] * len(waterbirds_water))
            assert (
                len(waterbirds_water) == MAJORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files [waterbirds_water]. Expected {MAJORITY_SIZE[self.split]} files got {len(waterbirds_water)}"
    ...
    def _download_data(self):
        ...
        with open(self.filename, "wb") as file:
            for chunk in tqdm(
                response.iter_content(chunk_size=1024), total=2952065, desc="Downloading SpuCoBirds", unit="KB"
            ):
                file.write(chunk)
