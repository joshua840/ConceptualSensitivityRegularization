...
from . import TEST_SPLIT, TRAIN_SPLIT, VAL_SPLIT, BaseSpuCoDataset, SourceData, SpuriousFeatureDifficulty
from .spuco_mnist_config import config
...
class SpuCoMNIST(BaseSpuCoDataset):
    ...
    def load_data(self) -> SourceData:
        ...
        self.mnist = torchvision.datasets.MNIST(
            ...
            transform=T.Compose(
                [T.ToTensor(), T.Lambda(lambda x: torch.cat([x, x, x], dim=0))]  # convert grayscale to RGB
            ),
        )
        ...
        if self.split == TRAIN_SPLIT:
            self.mnist = Subset(
                dataset=self.mnist, indices=[i for i in range(len(self.mnist)) if i not in config[VAL_SPLIT]]
            )
        elif self.split == VAL_SPLIT:
            self.mnist = Subset(dataset=self.mnist, indices=config[VAL_SPLIT])

        self.data = SourceData(self.mnist)

        # Validate Classes
        assert SpuCoMNIST.validate_classes(
            self.classes
        ), "Classes should be disjoint and only contain elements 0<= label <= 9"
        ...
        if self.split == TRAIN_SPLIT or (self.split == VAL_SPLIT and self.spurious_correlation_strength != 0):
            assert (
                self.spurious_correlation_strength != 0
            ), f"spurious correlation strength must be specified and > 0 for split={TRAIN_SPLIT}"
            ...
            if self.core_feature_noise > 0:
                self.data.core_feature_noise[
                    torch.randperm(len(self.data.X))[: int(self.core_feature_noise * len(self.data.X))]
                ] = 1
            ...
            for label in tqdm(self.partition.keys(), desc="Adding spurious feature", disable=not self.verbose):
                ...
                is_spurious[
                    torch.randperm(len(self.partition[label]))[
                        : int(spurious_correlation_strength * len(self.partition[label]))
                    ]
                ] = 1
                ...
                background_label = torch.tensor(
                    [
                        label if is_spurious[i] else other_labels[i % len(other_labels)]
                        for i in range(len(self.partition[label]))
                    ]
                )
                ...
                for i, idx in enumerate(self.partition[label]):
                    ...
                    background = SpuCoMNIST.create_background(
                        self.spurious_feature_difficulty, self.colors[self.data.spurious[idx]]
                    )
                    ...
                    if self.data.core_feature_noise[idx]:
                        core_feature_noise = (
                            (torch.randn_like(self.data.X[idx][0, :, :]) > 0.25).unsqueeze(dim=0).repeat(3, 1, 1)
                        )
