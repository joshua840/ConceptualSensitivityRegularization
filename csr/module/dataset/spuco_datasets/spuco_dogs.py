...
from . import TEST_SPLIT, TRAIN_SPLIT, VAL_SPLIT, BaseSpuCoDataset, SourceData
...
class SpuCoDogs(BaseSpuCoDataset):
    ...
    def __init__(
        ...
        super().__init__(root=root, split=split, transform=transform, num_classes=2, verbose=verbose)
    ...
    def load_data(self) -> SourceData:
        ...
        try:
            ...
            self.data.X.extend(
                [str(os.path.join(self.dset_dir, f"{SMALL_DOGS}/{INDOOR}", x)) for x in small_dogs_indoor]
            )
            self.data.labels.extend([0] * len(small_dogs_indoor))
            self.data.spurious.extend([0] * len(small_dogs_indoor))
            assert (
                len(small_dogs_indoor) == MAJORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files. Expected {MAJORITY_SIZE[self.split]} files got {len(small_dogs_indoor)}"

            # Small Dogs - Outdoor
            small_dogs_outdoor = os.listdir(os.path.join(self.dset_dir, f"{SMALL_DOGS}/{OUTDOOR}"))
            self.data.X.extend(
                [str(os.path.join(self.dset_dir, f"{SMALL_DOGS}/{OUTDOOR}", x)) for x in small_dogs_outdoor]
            )
            self.data.labels.extend([0] * len(small_dogs_outdoor))
            self.data.spurious.extend([1] * len(small_dogs_outdoor))
            assert (
                len(small_dogs_outdoor) == MINORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files. Expected {MINORITY_SIZE[self.split]} files got {len(small_dogs_outdoor)}"
            ...
            assert (
                len(big_dogs_indoor) == MINORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files. Expected {MINORITY_SIZE[self.split]} files got {len(big_dogs_indoor)}"

            # Big Dogs - Outdoor
            big_dogs_outdoor = os.listdir(os.path.join(self.dset_dir, f"{BIG_DOGS}/{OUTDOOR}"))
            self.data.X.extend(
                [str(os.path.join(self.dset_dir, f"{BIG_DOGS}/{OUTDOOR}", x)) for x in big_dogs_outdoor]
            )
            self.data.labels.extend([1] * len(big_dogs_outdoor))
            self.data.spurious.extend([1] * len(big_dogs_outdoor))
            assert (
                len(big_dogs_outdoor) == MAJORITY_SIZE[self.split]
            ), f"Dataset corrupted or missing files. Expected {MAJORITY_SIZE[self.split]} files got {len(big_dogs_outdoor)}"
            ...
            if self.label_noise > 0.0:
                ...
                self.data.labels = [
                    1 - label if self.is_noisy_label[i] else label for i, label in enumerate(self.data.clean_labels)
                ]
        except:
    ...
    def _download_data(self):
        ...
        with open(self.filename, "wb") as file:
            for chunk in tqdm(
                response.iter_content(chunk_size=1024), total=2593935, desc="Downloading SpuCoDogs", unit="KB"
            ):
                file.write(chunk)
